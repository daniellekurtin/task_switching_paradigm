<<<<<<< HEAD:past_data_and_docs/behav_analysis.py
import pandas as pd
import numpy as np
...
for title, df_mrt1 in df_mrt.groupby(level=[0, 1]):
    df_mrt1 = df_mrt1.apply(pd.to_numeric, errors = 'coerce').dropna(how = 'all')
    mask = df_mrt1.index.get_level_values(2)
    mrt = df_mrt1.groupby(level=[0, 1, 2]).mean()
    #print('\nPLOTTING MRT (participant_id, type) = {}'.format(title))
    ...
    for i in mrt:
        ...
        plt.savefig(dest)  
        plt.cla()
    df_mrt1.reset_index(drop = True, inplace = True)
    print('\nYOU GOT THIS GURL SMASH THE ANALYSIS')
...
# Make the Occurence Column 
raw_data_location = open(r'C:\Users\danie\Documents\SURREY\Project_1\task_switching_paradigm\data\public\ExperimentTaskSwitch-v0.0.4_trials-v0.0.1.csv')
df = create_df(raw_data_location)
df.to_csv(r'pilot3_withoccurence.csv')
>>>>>>> 3620e7edc2e8cd7626c956437002d78e28dc7247:data/behavanalysis_part1.py
