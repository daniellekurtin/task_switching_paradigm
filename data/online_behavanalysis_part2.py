...
def create_df2(raw_data_location2):
    ...
    df = pd.read_csv(raw_data_location2, header = 0)
    ...
    zz = xx - yy
    print('NUMBER OF TRIALS MISSED', zz)
    # number of trials missed as a % out of total
    vv = zz / xx * 100
    print('PERCENT OF TOTAL TRIALS MISSED', vv)

    no_participants = df['auto_participant_id'].nunique()
    print('NUMBER OF PARTICIPANTS', no_participants)

    df_age = df.drop_duplicates(subset='auto_participant_id', keep='last', inplace=False)
    mean_age = df_age['participant_age'].mean()
    print('AVERAGE PARTICIPANT AGE=',mean_age)

    xx = df_age['participant_gender'].value_counts()
    print('NUMBER OF MALES AND FEMALE =', xx)    

    df.groupby('auto_participant_id')
    ...
    df1.set_index(['auto_participant_id'], inplace = True)

    for group_index, group_value in df1.groupby(level=[0]):
    ...
    df2.set_index(['auto_participant_id', 'type'], inplace = True)
